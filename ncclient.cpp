...
server_session_info::server_session_info(sockaddr_in addr, unsigned char blk_size): _ADDR(addr),_MAX_BLOCK_SIZE(blk_size)
{
...
ncclient::ncclient(int socket, std::function<void (unsigned char *, unsigned int, sockaddr_in addr)> rx):_SOCKET(socket), _receive_callback(rx){}
...
void ncclient::_rx_handler(unsigned char* buffer, unsigned int size, sockaddr_in* sender_addr, unsigned int sender_addr_len)
{
    ...
    if(lookup_result == nullptr)
    {
        try
        {
            session_info = new server_session_info((*sender_addr), GET_OUTER_MAX_BLK_SIZE(_rx_buffer));
        }
        catch(std::exception ex)
        {
            return;
